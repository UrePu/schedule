...
| 보스 | 이유 |
|---|---|
| ~~**메이린**~~ | ⚠️ **2026-08-25 발주자 결정으로 제외를 뒤집었다** — *"메이린도 기록 해 시즌이지만 도는 보스잖아."* 아래 별도 절 참조. |
| **우르스** | 일 3회 입장, **최대 18인 파티**, 강렬한 힘의 결정을 주지 않는다. 주간 숙제 트래커에는 넣을 수 있으나 결정석 수익 대상이 아니다. |
| 필드 보스 / 스토리 전용 보스 / 파티 퀘스트 보스 | 결정석 미지급 |

#### ⚠️ 메이린은 제외에서 **들어왔다** (2026-08-25, 마이그레이션 43)

발주 지시: *"메이린도 기록 해 시즌이지만 도는 보스잖아."* 실제로 도는 보스이므로 화면에
나와야 한다. 다만 **원래 근거 중 하나는 여전히 참이고, 그것이 설계의 핵심이 됐다.**

- **주간 12칸에 포함되지 않는다 — 발주자 확정(2026-08-26).**
  원문: *"메이린은 12칸에 안막혀. 주간보스는 12개를 돌아도 추가로 메이린을 잡을수있음"*.
  그전까지는 나무위키 기반 **추정**이었고 근거도 간접적이었다(12/12 인 캐릭터에서
  `registration_flag` 가 살아 있더라 — 등록은 체크박스일 뿐이라 입장 가능의 증거가
  아니다). 이제 **입장 자체가 막히지 않는다**는 것이 직접 확인됐다.
  그래서 `cycle = 'weekly'` 로 넣되
  `boss_difficulties.counts_toward_weekly_limit = false` 를 준다. 실측이 이를 뒷받침한다 —
  킴잔델의 스냅샷이 `weekly_boss_clear_count 12 / limit 12` 인데 하드 메이린은
  `registered = true` 였다. 12칸이 찬 뒤에도 등록이 살아 있다는 것은 그 12에 들어가지
  않는다는 뜻이다(2025-08-21 패치 이후 13번째 주간 보스는 입장 자체가 막힌다).
- **결정석 시세 — 노멀 3억 / 하드 6억** (발주자 확인, 2026-08-25 · 마이그레이션 44).
  등록 당시에는 공개 가격표에 없어 `null`(미상)이었다(`review-BOSS-DATA.md` L96).
  **다른 보스와 똑같이 다룬다** — 발주 지시: *"분리하지말고 그냥 결정석 가격에 넣어"*.
  별도의 보상 종류나 예외 경로를 만들지 않았으므로 `floor(가격 / 인원)` 규칙도 그대로다.
  ★ R3(새 효력 시각으로 얹기)의 **좁은 예외**로 null 행을 제자리에서 채웠다. 클리어가
  0건이라 지킬 과거가 없고, 값이 *바뀐* 것이 아니라 몰랐던 것을 알게 된 것이라 새 효력
  시각을 만들면 없던 사건을 기록하게 되기 때문이다. 44번 자기검증이 클리어 0건을 강제한다.
- **`max_party = 3` 은 추정이다.** modern 세대에 맞춘 값이고 개별 출처가 없다(D5).
  경고용 soft bound 라 실제 파티를 막지 않는다.
- **2026-09-16(수) 23:59 입장 종료.** 그날이 지나면 넥슨 목록에서 빠지므로 계획도 자연히
  갱신된다. 만료 장치는 두지 않았다.
...
- **2026-09-16 — 메이린(챌린저스 월드 이벤트 보스) 입장 종료.** 2026-08-25 부로 앱에 **넣었으므로**(마이그레이션 43) 이 날짜에 노출을 정리할 것. 넥슨이 목록에서 빼면 계획은 자동으로 따라온다.
