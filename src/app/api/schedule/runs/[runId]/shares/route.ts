 ...
 *
 * ⚠️ **부르는 화면이 없다 — 새 코드는 `parties/{partyId}/shares` 를 쓸 것**
 *    (2026-08-19 발주자: *"분배조율도 파티 설정에 있어야된다고 했잖슴"*).
 *    분배는 마이그레이션 `20260819200000` 이후 **파티**가 갖는다. 이 라우트도 결국 같은
 *    컬럼(`parties.share_mode` · `party_participants.share_bp`)에 쓰므로, 런 하나를
 *    가리켜 놓고 실제로는 **그 파티 전체**를 바꾼다. 화면(`RunShareEditor`)은 그래서
 *    지웠다. 라우트를 남긴 이유는 과거 클라이언트가 있을 수 있어서일 뿐이며, 여기에
 *    기능을 더하지 말 것.
 *
