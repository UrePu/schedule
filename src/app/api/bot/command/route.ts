...
/**
 * **`!환산` 이 헤드리스 크롬을 띄운다**(2026-09-03, `features/bot/server/scouter.ts`).
 *
 * 다른 명령은 전부 DB 왕복 몇 번이라 기본 한도로 충분했는데, 이 하나만 성격이 다르다:
 * 크롬 기동 + 페이지 열기 + 응답 대기라 서버리스 콜드스타트가 겹치면 기본 한도를 넘긴다.
 * 넘기면 방에는 **아무 말도 나가지 않는다** — 사람 눈에는 봇이 죽은 것으로 보이고,
 * 카드조차 안 뜬다.
 *
 * ⚠️ 이 값은 **답장이 늦어도 된다는 뜻이 아니다.** 조회 자체의 상한은 `scouter.ts` 가
 *    쥐고 있고, 그쪽 예산은 **조회 전체 25초 + 브라우저 종료 3초 = 28초**다(기동·CDP·
 *    응답 대기가 각각 따로 걸려 있고, 그 바깥을 25초짜리 예산 하나가 다시 덮는다).
 *    여기 60은 그 28초가 잘리지 않게 하는 여유다 — `28 < 60` 이 성립해야 하므로,
 *    **`scouter.ts` 의 예산을 올릴 사람은 이 부등식을 먼저 확인할 것.**
 *    ⚠️ 초판은 이 자리에 "타임아웃 12초" 라고 적었는데 **거짓이었다.** 그 12초는 응답
 *       대기에만 걸려 있었고 기동(최대 30초)·CDP(최대 180초)는 무방비였다.
 */
