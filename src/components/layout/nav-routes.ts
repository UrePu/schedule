...
export const NAV_GROUPS: readonly NavGroup[] = [
  ...
  },
  {
    ...
    routes: [
      /*
        ★ 파티 관리와는 **2026-08-25 에 갈라졌다**(발주 지시: *"일정짜기를 두가지로
          분리하자"*). 한 화면이 "누구와 무엇을"과 "언제"를 동시에 물어서 헷갈렸다.

        ★ **이름과 순서가 2026-09-01 에 바뀌었다**(발주 지시: *"일정 관리 > 일정 계획으로
          바꾸고 파티관리 위에다가 올겨달라고"*).
          · `관리` 가 세 번 나오면 그 단어로는 아무것도 갈리지 않는다. 이 화면은 고치는
            곳이 아니라 **짜는** 곳이라 `계획` 이 하는 일을 더 정확히 말한다.
          · 순서는 **매일 여는 것이 위**로 뒤집었다. "파티를 먼저 만든다"는 처음 한 번이고,
            그 뒤로는 일정만 잡는다. 메뉴 순서는 설치 순서가 아니라 **빈도 순서**여야 한다.
      */
      {
        href: "/schedule",
        label: "일정 계획",
