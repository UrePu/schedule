 ...
 *
 *     현황  ├ 이번주 일정  ├ 계정 보스 현황  └ 기타 숙제
 *     관리  ├ 일정 추가    ├ 캐릭별 보스 관리 └ etc
 *
 ...
 *
 * 항목이 4 → 8 로 늘었으므로 평평한 한 줄은 더 이상 성립하지 않는다. 접는 방식을
 * 무리 단위로 고른 이유는, 사람이 "수익 보러 왔다"를 먼저 정하고 그다음 화면을 고르기
 * 때문이다. 여덟 개를 알파벳순으로 늘어놓는 것보다 두 번 고르는 쪽이 빠르다.
 *
