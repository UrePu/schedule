 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 데스크톱은 **hover 로 연다** — 나갈 때는 짧은 유예 (발주 지시 2026-08-20)
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 *      · "지나가다 열리는 사고가 잦다" → **닫는 쪽에 유예**를 두면 실제로 문제가 되는
...
/**
 * hover 가 메뉴에서 빠져나간 뒤 닫히기까지의 유예(ms).
 *
 * 이 값이 하는 일은 **대각선 이동을 허용하는 것**이다. 트리거에서 메뉴 아래쪽 항목으로
 * 곧장 내려가면 도중에 둘 다에서 잠깐 벗어나는데, 유예가 없으면 그 순간 닫힌다.
 * 그 이동에 필요한 시간은 100ms 안팎이라 500ms 면 넉넉하다.
 *
 * 발주자가 실제로 써 보고 정한 값이다 — 처음 지시는 1초였고(2026-08-20:
 * *"빠져나가는것도 1초의 대기시간을 주고나서 꺼지게 해"*), 만져 본 뒤 *"1초는 너무 길다
 * 0,5초"* 로 정정했다. 유예는 길수록 안전한 값이 아니다: 메뉴를 떠났는데도 계속 떠
 * 있으면 아래에 있는 것을 가리고, 사용자는 자기가 뭔가 잘못 눌렀다고 생각한다.
 */
      ...
      {isOpen ? (
        /*
          ★ 바깥 껍데기가 `pt-1` 을 갖는다. 예전에는 메뉴 자체가 `mt-1` 로 4px 떨어져
            ...
            메뉴로 내려가는 길에 잠깐 둘 다에서 벗어나 닫기 타이머가 걸린다. 유예가
 ...
 *
 * hover 로 열고 `HOVER_CLOSE_MS` 뒤 닫는다(파일 머리말). 무리 사이를 옮겨 갈 때는 **새 무리를 바로
