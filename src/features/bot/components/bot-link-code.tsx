 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 코드가 **여러 종류**라는 사실이 이 컴포넌트의 존재 이유다
 * ─────────────────────────────────────────────────────────────────────────────
 *   `channel_pair` — **파티방**을 서버에 붙인다. 방마다 최초 1회. 클라이언트가 `!페어링` 으로 쓴다.
 *   `member_link`  — **사람**을 그 방에서 식별한다. 사람마다 최초 1회. `!연결` 로 쓴다.
 *   `direct_pair`  — **개인톡** 방을 붙인다(2026-08-31). 소모 경로는 `channel_pair` 와
 *                    같지만 그렇게 열린 방만 그 사람의 **모든** 일정 알림을 받는다.
 *                    허용 명단에 없으면 서버가 발급을 거절한다 — 버튼을 아직 어느
 *                    화면에도 붙이지 않은 이유이기도 하다(웹 UI 는 이번 범위 밖).
 *
...
> = {
  ...
  direct_pair: {
    button: "개인톡 연결 코드",
    usage: (code) => `개인톡 방에서 !페어링 ${code} 를 입력하세요.`,
  },
};
