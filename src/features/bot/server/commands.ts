 ...
 *      `user_id` 로만 쓴다.
 * ★ **한글을 퍼센트인코딩하지 않는다.** 카카오톡·브라우저가 링크를 열 때 알아서
 *   인코딩하므로 평문에는 원문을 그대로 적는 편이 맞다. 이유는 **길이**다 — 한글 한
 *   글자가 인코딩되면 9자(`%EB%A9%94`)로 불어나 350자 평문 예산을 갉아먹고, 방에서
 *   사람이 무슨 닉네임인지 읽지도 못한다. URL 안전성은 인코딩이 아니라 아래 검증
 *   정규식이 보장한다 — `&` `#` `?` 공백은 애초에 통과하지 못한다.
...
function handleScouter(
  ...
  */
  ...
  /*
    ★ 이 정규식이 곧 **URL 안전성 보장**이다. 한글·영문·숫자 2~12자만 통과하므로
      질의문자열을 깨뜨릴 문자가 링크에 들어갈 길이 없다.
  */
  ...
  return {
    reply: lines(`🔍 ${nickname} 환산`, `${SCOUTER_BASE}${nickname}`),
    tag: "환산",
    userId,
  };
}
