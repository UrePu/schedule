 ...
 * 어느 주기를 담는가.
 *
 * · `"weekly"`(기본) — 주간 + **시즌**. 둘 다 목요일에 초기화되므로 "이번 주에 뭘 더
 *   돌까"의 답이 같은 자리에 온다.
 * · `"monthly"` — 월간만. 실질적으로 검은 마법사다.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 왜 섞지 않고 **가르는가** (2026-09-02 발주 지시)
 * ─────────────────────────────────────────────────────────────────────────────
 * *"!숙제 이거 검마 월간 제외해서 !검마 이걸로 전부 이동. !숙제는 주간만"*
 *
 * 같은 날 오전에는 한 목록에 섞었다(*"월간도 보여주게"*). 실제로 그렇게 내보내니
 * **87억짜리 익검이 상위 다섯 줄을 통째로 차지했다** — 15줄 중 3분의 1이 한 보스이고,
 * 그 보스는 이번 주에 안 해도 사라지지 않는다. 2026-08-25 에 `!결정석` 상위 3개에서
 * 월간을 뺐던 이유가 15줄에서도 그대로 재현된 것이다.
 *
 * 초기화 시계가 다른 것을 한 목록에 두면 **정렬 기준(금액)이 두 종류의 급함을 섞는다.**
 * 주간은 "이번 주 안에", 월간은 "이번 달 안에"이고, 금액만으로 줄을 세우면 언제나
 * 월간이 이긴다. 그래서 명령을 가른다 — `!숙제`(주간) · `!검마`(월간).
 */
export type RemainingBossScope = "weekly" | "monthly";
...
export async function fetchRemainingBosses(
  ...
  for (const row of rows) {
    ...
    /*
      부르는 쪽이 고른 주기만 담는다(`RemainingBossScope` 머리말).
      일간은 추적 범위 밖이라 어느 쪽에도 들어오지 않는다(2026-08-18 발주자 결정).
      ⚠️ 시즌이 별도 주기가 된 2026-08-26 에 `=== "weekly"` 하나였다면 메이린이 주간
         목록에서 **조용히 사라졌을** 것이다 — 초기화가 주간이라 거기 있어야 한다.
    */
    const inScope =
      (options.scope ?? "weekly") === "monthly"
        ? entry.cycle === "monthly"
