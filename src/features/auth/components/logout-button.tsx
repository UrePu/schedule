 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * `/` 는 **서버에서** 세션을 보고 대시보드/랜딩을 가르므로, 쿠키만 지우고 서버 렌더를
 * 새로 받지 않으면 로그아웃했는데도 대시보드가 그대로 남는다. 예전에는 `router.refresh()`
 * 로 그걸 했는데, 이 경로는 로그인 경로보다 **더** 확실해야 한다:
 *
 * - `router.refresh()` 는 트랜지션이 커밋될 때까지 옛 UI(=대시보드)를 그대로 보여 준다.
 ...
 *
 * `assign` 이 아니라 `replace` 인 이유: 뒤로가기가 방금 버린 대시보드로 돌아가면 안 된다.
 * 이 버튼은 `Dashboard` 에만 있고 `Dashboard` 는 `/` 에만 있으므로 목적지는 `/` 다.
 */
