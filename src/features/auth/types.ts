...
export type ApiErrorKind =
  ...
  | "bad_request"
  /**
   * 봇 채널 인증 실패 — **모르는 방 · 서명 불일치 · 시각 창 초과를 하나로 접는다.**
   *
   * 세 가지를 갈라 주면 "그 `room` 은 존재한다"가 새어 나가고, 불투명 ID 를 훑어
   * 남의 방 존재 여부를 확인할 수 있게 된다(초대 토큰이 원인을 나누지 않는 것과 같은
   * 이유). 상태 코드는 401/404 로 나뉘지만 `kind` 는 하나다.
   */
  | "bot_unauthorized"
  /** 같은 `nonce` 가 다시 왔다. 리플레이이므로 처리하지 않는다(409). */
  | "bot_replay"
  /**
   * 방/발신자 레이트리밋 초과(429).
   *
   * ⚠️ 넥슨의 `quota_exceeded` 와 **섞지 않는다.** 그쪽은 "넥슨 하루 한도"라는 전혀
   *    다른 사건이고 사용자 문구도 다르다.
   */
  | "bot_rate_limited"
  /** 우리 쪽 오류. */
