  ...
  return (
    <Card className={cn("flex flex-col gap-3", className)}>
      ...
      {/*
        ...
        금액은 compact 표기(`428억`)를 써서 셋이 한 줄에 들어가게 한다.

        ⚠️ **모바일에서는 세로로 쌓는다**(발주 지시 2026-08-28: *"데스크탑에선 저거 그대로.
           반응형으로 모바일때 다 짤리니까 세로배치"*).
           예전 주석은 "좁은 화면에서도 3열을 유지한다 — 세로로 쌓으면 한눈에 비교라는
           이 카드의 유일한 일이 사라진다"였다. 의도는 맞았지만 결과가 반대였다:
           360px 화면에서 칸 하나가 ~100px 로 눌려 `3건 / 최대 428억` 이 **`3건 / 최대 …`**
           로 잘렸다. 잘린 숫자는 비교할 수 없으므로, 3열을 지킨 대가로 정작 그 "비교"를
           잃고 있었다. 세로로 쌓으면 칸이 화면 폭을 다 쓰므로 **아무것도 안 잘린다** —
           비교는 한 줄 아래로 내려갈 뿐 사라지지 않는다.
        ★ `sm`(640px)에서 3열로 돌아온다. 데스크탑은 지시대로 그대로다.
        ★ 이 저장소의 다른 3칸 격자(`crystal-income-summary` · `week-ledger-list` ·
          `drop-record-form`)는 전부 이미 `grid-cols-1 … sm:grid-cols-3` 이었다.
          여기만 예외였던 것이라, 맞추는 쪽이 일관성에도 맞다.
      */}
