...
async function recordApiClears(
  ...
  /*
    ═══════════════════════════════════════════════════════════════════════════
    주차는 **명목 시각이 정한다** — 목요일 경계에서 이게 갈린다
    ═══════════════════════════════════════════════════════════════════════════
    주간 초기화는 KST 목요일 00:00 이다(§1). 그런데 밤 크론은 Hobby 오차(±59분) 때문에
    수요일 밤에 걸어 둔 작업이 **목요일 새벽에 뜰 수 있다.** 그때:

      observedAt(넥슨 date) = 목 00:00  → 다음 주차
      명목 시각             = 수 23:55  → 지난 주차   ← 이쪽이 맞다

    발견된 클리어는 **수요일에 잡은 것**이다. 넥슨 데이터가 ~15분 늦어 아직 초기화 전
    상태를 보고 있기 때문이고, 만약 초기화가 이미 반영됐다면 `complete_flag` 가 전부
    false 라 **애초에 기록할 것이 없다.** 즉 이 분기가 실제로 도는 순간은 언제나
    "초기화 전 데이터를 보고 있다"이므로, 명목 시각의 주차를 따르는 것이 옳다.

    ⚠️ 예전에는 여기서 반대로 했다 — 주차가 갈리면 명목을 버리고 `observedAt` 으로
       되돌렸다. 그러면 수요일 클리어가 **다음 주 수익으로 넘어가고** 주간 12칸까지
       어긋난다. 발주 지적(2026-08-25): *"목요일 넘어갈때만좀 확실히 해야될거같은데."*

    ★ 명목 시각이 **없는** 경로(진입 시 동기화 · 런 종료 후 · 수동)는 그대로 관측 기준
      주차를 쓰고, 호출 시각이 그 주차를 벗어나면 관측 시각으로 되돌린다. 그쪽은 이
      작업처럼 "어느 날을 대표한다"는 약속이 없어 명목이라 부를 값이 없다.
  */
  const weekKey = getWeekKey(clearedAtOverride ?? observedAt);
