...
/**
 * 사용자 **전체**의 계획 행. 주간 체크리스트가 쓴다.
 *
 * ★ **추적 조건은 여기에 없다 — 일부러 없다**(문장 정정 2026-09-22). 예전 주석은 "추적
 *   캐릭터 전원"이라고 말했지만 질의에 그 조건이 없어서, 읽는 사람이 이 함수가 걸러 준다고
 *   믿게 만들었다. 실제로는 추적 해제된 캐릭터와 넥슨 목록에서 사라진 캐릭터의 계획 행까지
 *   **함께 싣는다**(실측 2026-09-22: 한 계정에서 29행. 이 함수는 `is_active`·`is_cleared` 를
 *   보지 않으므로 봇 쪽 숫자 22행보다 꺼 둔 계획 7행이 더 많다).
 * ★ 유령을 빼는 일은 `fetchWeeklyChecklist` 가 한다 — 카드를 먼저
 *   `fetchTrackedChecklistCharacters`(`is_tracked = true` ∧ `missing_since is null`)로
 *   만들고, 여기서 온 행을 **그 명단에 붙이기만** 하므로 명단에 없는 캐릭터의 행은 갈 곳이
 *   없어 버려진다. 그래서 이 경로의 동작은 이미 옳고, 조건을 여기에 또 넣지 않는다.
 * ⚠️ 이 함수의 결과를 **명단 없이 그대로 나열하면 유령이 그대로 나온다.** 봇의
 *   `fetchRemainingBosses` 가 정확히 그 사고였다(2026-09-22 — 그쪽은 `characters` 임베드로
 *   직접 거른다). 같은 뷰를 새로 읽는 코드를 쓸 때는 둘 중 어느 쪽인지 먼저 정할 것.
 */
