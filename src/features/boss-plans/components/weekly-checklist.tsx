...
function partyGlyphIndexes(partySize: number): readonly number[] {
  return Array.from(
    { length: Math.min(Math.max(partySize, 1), 6) },
    (_, i) => i,
  );
}
  ...
  */
  ...
  /*
    ...
    ★ **"주간"의 기준은 `cycle === "weekly"` 다 — `countsTowardWeeklyLimit` 이 아니다.**
      ...
      와 무관하게 주 단위로 다시 도는 보스는 전부 벌이에 들어가야 한다.
      실측(`src/lib/boss-master/generated.ts`, 2026-09-03): `BossCycle` 은
      `daily | weekly | monthly | season` 네 값이고, 마스터 80행의 분포는
      daily 24 · weekly 52 · monthly 2 · season 2 다. **`cycle: "weekly"` 이면서
      `countsTowardWeeklyLimit: false` 인 행은 0개**이고, 메이린 2행은
      `cycle: "season" · counts=false` 로 들어가 있다. 즉 오늘은 두 기준이 같은 집합을
      내므로 화면상 차이가 없다 — 그럼에도 `cycle` 을 쓰는 이유는, 언젠가 "주간인데
    ...
    <Card className="@container flex h-full flex-col gap-2">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        */}
        <div className="flex shrink-0 flex-col items-end gap-1.5">
          ...
          {/*
            `[&_button]:w-full` — 세로로 쌓으면 두 버튼의 자연 폭이 달라(`새로고침` >
            `수정`) 오른쪽 끝은 맞고 왼쪽 끝이 어긋난다. 넓은 쪽에 맞춰 늘려 **두 변이
            모두 정렬**되게 한다. `w-fit` 이라 늘어나는 기준은 넓은 버튼 자신이다.
            (`SyncButton` 은 className 을 받지 않아 자식 선택자로 건다 — 그 하나 때문에
            호출부 두 곳의 시그니처를 바꿀 값이 없다.)
          */}
          <div className="flex w-fit flex-col items-stretch gap-1.5 [&_button]:w-full">
            {/*
      ...
      */}
      ...
      {/*
        ...
        이 화면이 답하는 질문은 "이번 주 보스가 얼마나 남았나" 하나다.
      */}
...
export function WeeklyChecklist({ className }: WeeklyChecklistProps) {
   ...
   * ★ 응답으로 온 체크리스트는 그대로 캐시에 심는다 — `N/12` · 격자 · `현재 / 최대` 금액이
      ...
      */}
      ...
      {!autoSync.isSyncing && missingKeyEntries.length > 0 ? (
        <div className="flex flex-wrap items-start gap-2 rounded-md border border-chip-soon-border bg-chip-soon-bg px-3 py-2">
          ...
          />
          <p className="min-w-0 flex-1 text-body-sm text-ink">
            {missingKeyNames} — {missingKeySentence} 아래 내용은 마지막으로
            저장된 값입니다.
          </p>
          {/*
      ...
      {!autoSync.isSyncing && autoSync.summary?.failure != null ? (
        <p className="flex items-start gap-2 rounded-md border border-chip-soon-border bg-chip-soon-bg px-3 py-2 text-body-sm text-ink">
          ...
          />
          <span>
            자동 갱신을 마치지 못했습니다 — {autoSync.summary.failureMessage}{" "}
            아래 내용은 마지막으로 저장된 값입니다.
      ...
      */}
      ...
      {sync.isSuccess && !sync.isPending ? (
        <p className="rounded-md border border-border bg-neutral-100 px-3 py-2 text-body-sm text-ink-label">
          {sync.data.characterName} 동기화 완료 — 보스{" "}
          {sync.data.bossEntryCount}건 확인 · 계획 {sync.data.planUpdatedCount}
          건 · 클리어 {sync.data.clearRecordedCount}건 반영
      ...
      {checklistQuery.isError ? (
        ...
        /*
          ...
          *"카드 가로길이 줄이라고 배치 바꿔"*). 12칸 격자가 4열 × 69px + 간격 = 288px,
          카드 좌우 여백까지 **320px 이 한 카드의 하한**이다.
          ⚠️ 3단을 lg(1024)가 아니라 **xl 로 미뤘다.** 좁은 셸에서 lg 는 카드당 323px 로
             하한(320px)과 3px 밖에 차이가 안 났다. 이보다 이르게 단을 늘리면 격자가
             눌려 아이콘이 칸 밖으로 나간다.

          ── 2026-09-04: 그리드가 아니라 **셸이 넓어졌다** ─────────────────────
          발주 지시: *"이번주 체크리스트의 각 카드의 가로길이를 더 길게 해줘"*.
          `md:2단 · xl:3단` 은 **한 줄도 바뀌지 않았다.** 손댄 곳은 `/boss-status`
          페이지의 셸 폭이다 — `PAGE_SHELL_CLASS`(max-w-6xl, 1152px) →
          `WIDE_PAGE_SHELL_CLASS`(max-w-[92rem], 1472px). 셸 좌우 여백 16px(md 이상
          24px), 카드 간격 `gap-3`(12px)로 계산한 실제 카드 폭:

            390       1단 358px
            768 (md)  2단 354px      1024 (lg) 2단 482px
            1280(xl)  3단 403px      1536 (2xl) 3단 **466px**  ← 폭 상한 1472px 이 걸린 뒤

          예전 3단은 1152px 셸에서 360px 이었으니 **466px 로 +29%**. 어느
          브레이크포인트에서도 하한 320px 아래로 눌리지 않는다.
          ★ 그래도 **4단은 여전히 금지**다(아래 문단). 넓어진 폭을 단으로 쓰면
            2xl 에서 (1424-36)/4 = 347px 로 카드가 도로 좁아져, 없애라고 한 그 배치가
            그대로 돌아온다. 남는 폭은 **단이 아니라 카드**가 먹는다.
        */
        /*
